...
public class SortShow extends JPanel { 
		...
		public void SelectionSort(){
			...
			int low_index; //value to hold the lowest index
			int j = 0;
			do {
				//find the smallest number's index
				low_index = getIndexOfSmallest(j, total_number_of_lines);
				//then swap them
				swap(j, low_index);
		...
		public int getIndexOfSmallest(int first, int last){
			...
			for (int i = first + 1; i < last; i++) {
				if (lines_lengths[low_index] > lines_lengths[i]) {
					low_index = i;
				}
			}
			return low_index;
	...
	public void QuickSort(){
		...
		quickSortIteration(0, total_number_of_lines-1);
	...
	public int partition(int low, int high){
			int pivotPoint = lines_lengths[high];
	...
	public void quickSortIteration(int low, int high)
	{
		if(low < high){
